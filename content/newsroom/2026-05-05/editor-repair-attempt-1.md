...
- 동작 변경: 하이브리드 추론(온디바이스 및 클라우드 추론 활용) 및 이미지 생성을 위한 최신 Nano Banana 모델을 포함한 새로운 Gemini 모델 지원

**배경지식**

Android에서 AI 모델을 실행하는 방식은 온디바이스 추론과 클라우드 추론으로 나뉩니다. 하이브리드 추론은 이 두 가지 방식을 결합하여 성능과 유연성을 최적화합니다. Firebase AI Logic API는 개발자가 이러한 추론 방식을 앱에 통합할 수 있도록 돕는 도구입니다. Gemini 모델은 Google의 최신 AI 모델 제품군으로, 다양한 모달리티를 지원하며, 특히 Nano Banana 모델은 이미지 생성 기능을 제공합니다.

**Camera HAL 관점 해석**

이 변화는 카메라 HAL이 AI 워크로드에 대한 버퍼를 효율적으로 관리하고, NPU/GPU 리소스 경합을 최소화하며, 전력 소비를 최적화해야 함을 의미합니다. 하이브리드 추론은 온디바이스 추론 시나리오에서 HAL이 제공하는 프레임의 지연 시간, 처리량, 그리고 메타데이터 주석에 영향을 줄 수 있습니다. 이미지 생성 모델은 YUV 또는 RAW 스트림의 특정 해상도 및 형식 요구사항을 가질 수 있으며, 이는 HAL의 스트림 구성 및 버퍼 할당 전략에 영향을 미칠 수 있습니다.

**우리 팀이 확인할 Action Item**

- 2주 이내에 Firebase AI Logic API를 활용하는 온디바이스 AI 추론 시나리오에서 Preview + ImageAnalysis 스트림 조합의 프레임 드롭 및 캡처 지연 시간을 측정하는 테스트를 추가합니다.
- AI 분석 스트림 사용 시 NPU/GPU 스케줄링 및 전력 소비에 대한 HAL의 영향을 분석하고, 최적화 방안을 검토합니다.
- 새로운 Gemini Nano Banana 모델의 이미지 생성 기능이 HAL의 특정 스트림 형식 또는 해상도 지원에 미치는 영향을 확인하고, 필요한 경우 HAL 기능을 확장합니다.

**팀 공유용 한 줄**

Android용 하이브리드 추론 및 새로운 Gemini 모델(Nano Banana 포함) 지원은 카메라 HAL의 버퍼 관리, 성능, 전력 소비 및 스트림 구성에 직접적인 영향을 미 미치므로, 관련 API 동작 및 NPU/GPU 스케줄링을 면밀히 검토하고 테스트해야 합니다.
