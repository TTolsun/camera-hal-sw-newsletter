...
| --- | --- | --- | --- | --- |
| Android Developers Blog | 19 | 2 | main_eligible=false; source_gap_risk=true | 소스 유지, 파서 수정 |
| lore.kernel.org linux-media list | 12 | 0 | main_eligible=false; source_gap_risk=true | 소스 유지, 파서 수정 |
| ISO C++ Blog | 5 | 0 | main_eligible=false; source_gap_risk=true | 소스 유지, 파서 수정 |
...
| Multimedia bucket 추가 | `ADD_MULTIMEDIA_BUCKET` | 전체 | 9 camera-relevant candidate(s) were not mapped to a known camera bucket. | medium |
| Source discovery 중복 제거/수리 | `REPAIR_SOURCE_DISCOVERY_DUPLICATES` | 전체 | Gemini discovery produced 11 candidate(s) but gemini_new_unique_url_count=0.; Duplicate discovery signal detected: gemini_manual_duplicate_url_count=11, duplicate_discovery_gap_count=0. | medium |
