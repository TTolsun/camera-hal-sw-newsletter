...
| 파일 | 역할 |
| --- | --- |
| `news-sources.json` | 후보 수집 source registry의 machine-readable source of truth입니다. |
...
- `news-sources.json`의 `id`, `category`, `priority`, `reliability`, `collectionModeHint`, `schemaVersion` 같은 계약-bearing 값은 번역하지 않습니다.
...
- `docs/news-sources.md`는 사람이 검토하는 editorial view이고, machine source of truth는 `data/news-sources.json`입니다.
