...
| 파일 | 역할 |
| --- | --- |
| `newsletter-policy.json` | main article count, quality threshold, hard fail condition 같은 newsletter publication policy의 source of truth입니다. |
| `newsroom-budget.json` | newsroom 실행 budget과 비용 안전 관련 설정입니다. |

## 작업 규칙

- `README.md`의 Newsletter Policy block은 generated block입니다. 직접 손으로 고치지 말고 `config/newsletter-policy.json`을 수정한 뒤 policy docs sync를 확인합니다.
