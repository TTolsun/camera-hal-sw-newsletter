...
## 심층(deep-dive) 주제 큐 — 1단계(shadow)

파이프라인이 main으로 싣지 못한 Android 카메라 신호를 `state/deep-dive-topic-queue.json`에 적립합니다.

- 적립은 수집(01)이 합니다: 강등된 direct_aosp_camera·android 후보,
...
- feature 페이지 소스(`android-version-features`)는 심층 적립 전용입니다(`main_article_allowed: false`).
  ...
  버킷도 앱 개발자용 플랫폼 문서에 맞게 `android`입니다.
...
- 심층은 발행 결정 단계가 할 일을 **전부 마친 뒤** 맨 마지막에 실행합니다: 위클리 공개 산출물
  ...
  선점하지 못하게 하기 위해서입니다. 미발동·큐 빔 같은 예상 콘텐츠 실패는 report의 `skip_reason`으로
  남고 발행은 계속됩니다. 반대로 구현 오류(큐 JSON 손상 등)는 `status: "error"`로 기록한 뒤 다시
...
scoring은 다음을 함께 봅니다: `direct_aosp_camera`, `camera_driver_image_pipeline`, `cpp_ai_tooling_fallback`, `android`, `generic_tech_watchlist` bucket 분류와 더불어 구체 evidence, 최신성, 실무 actionability, source reliability.
...
- `00-review-guide.md`는 아래 순서로 review artifact를 묶습니다.
  ...
  7. 디버그 근거
  8. 미분류 산출물
- `artifact-manifest.json`은 `schema_version=5`입니다. version 4에서 `committed_artifacts[]` 항목이, version 5에서 `files[]`와 `review_artifacts[]` 항목이 `size`·`sha256`을 버렸습니다 — 커밋되는 파일의 바이트 정본은 Git tree이므로 매니페스트는 두 번째 정본을 두지 않습니다. `files[]` 항목은 `path`만 담고 실제로 존재하는 파일만 넣습니다. review 순서, 빠진 예상 artifact(missing expected artifact), `review_blocking`, `review_attention_required` 같은 안내성(advisory) metadata는 `files[]`가 아니라 `review_artifacts[]`에 둡니다.
...
- `artifact-manifest.json`은 파일 이름 기준으로 manifest `files[]`에서 제외합니다 — 자기 자신뿐 아니라 **모든** artifact-manifest가 빠집니다. 자기 자신의 내용은 자기 안에 기록할 수 없고, 다른 날짜의 매니페스트는 이 실행의 리뷰 대상이 아닙니다. `00-review-guide.md`와 `release-qa-report.md`는 존재하면 `files[]`에 포함될 수 있지만 derived artifact라서 `missingRequired` 판단에는 사용하지 않습니다.
- Manifest surface는 두 가지입니다.
  ...
  - snapshot root `artifact-manifest.json`: workflow snapshot manifest입니다. `.tmp/**`, cache, debug file 같은 snapshot file을 추가로 포함할 수 있으며 같은 `schema_version=5`와 review metadata field를 사용합니다. 이 manifest의 `files[]`도 `path`만 담습니다 — `size`·`sha256`을 읽는 소비자가 없었고(#1018), 이 배열에 섞이는 두 종류 어느 쪽도 사본을 둘 이유가 없기 때문입니다. 커밋되는 항목은 Git tree가 바이트 정본이라 매니페스트 사본이 곧 어긋나는 두 번째 정본이 됩니다. 커밋되지 않는 항목은 파일 자체가 이 매니페스트와 같은 `newsroom-final-debug-<run_id>` Actions artifact 안에 함께 들어가므로, 해시 사본이 따로 가리킬 정본이 없습니다. 어느 쪽인지는 경로 접두가 아니라 보존 등급이 가릅니다(`retentionCommitAllowlist`) — heavy 등급은 `articles/**` 아래에 있어도 커밋되지 않으므로 두 번째 갈래입니다. 그래서 해싱 대상도 heavy 등급(`debug_heavy`·`transient_attempt`) 파일로 좁혔습니다. heavy 목록만 `size`·`sha256`을 유지합니다 — 같은 모양이 커밋되는 date-scoped 매니페스트에도 실리는데 정작 그 파일은 PR diff에 없어서, 리뷰어가 Actions artifact에서 내려받은 파일이 그 실행의 것인지 확인할 수단이 이 값뿐입니다. 이 정리 이후 생성되는 매니페스트부터는 `schema_version=5`가 두 manifest surface에서 같은 `files[]` 모양 하나만 가리킵니다. 보존 기간(14일) 안에 남아 있는 기존 Actions artifact의 스냅샷 매니페스트는 여전히 `schema_version=5`에 `{path, size, sha256}`입니다.
...
NEWSROOM_WARN_COST_USD=0.5
NEWSROOM_MAX_COST_USD=0.7
```
...
현재 schedule entrypoint는 `Newsletters 00 - Weekly Orchestrator` (`.github/workflows/newsletters-00-orchestrator.yml`)입니다. cron(`0 0 * * 1`)을 가진 workflow는 00 하나뿐이고, 00이 collect(01) → discover(02) → generate(03)를 순서대로 호출합니다. 그보다 먼저 `published-guard` job이 대상 날짜나 그 주가 main에 이미 발행돼 있는지 보고, 발행돼 있으면 세 단계를 모두 돌리지 않습니다(#1167, 자세한 내용은 `docs/workflows/RAW_TO_GENERATE_ARTIFACT_CONTRACT.md`). 따라서 final newsletter generation도 예약 경로에서 자동 실행됩니다. 각 단계를 따로 돌리려면 `workflow_dispatch`로 수동 실행합니다.
