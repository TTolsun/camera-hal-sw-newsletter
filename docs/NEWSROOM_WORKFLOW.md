...
- `artifact-manifest.json` 파일 자신은 manifest `files[]`에서 제외합니다(자기 자신을 기록할 수 없습니다). `00-review-guide.md`와 `release-qa-report.md`는 존재하면 `files[]`에 포함될 수 있지만 derived artifact라서 `missingRequired` 판단에는 사용하지 않습니다.
- Manifest surface는 두 가지입니다.
  ...
  - snapshot root `artifact-manifest.json`: workflow snapshot manifest입니다. `.tmp/**`, cache, debug file 같은 snapshot file을 추가로 포함할 수 있으며 같은 `schema_version=5`와 review metadata field를 사용합니다. 이 manifest의 `files[]`만 `size`·`sha256`을 유지합니다 — snapshot directory는 커밋되지 않아 Git tree라는 정본이 없고, `retained_heavy_artifacts[]`가 그 값을 그대로 씁니다.
