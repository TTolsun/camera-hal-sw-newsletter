...
- Manifest surface는 두 가지입니다.
  ...
  - snapshot root `artifact-manifest.json`: workflow snapshot manifest입니다. `.tmp/**`, cache, debug file 같은 snapshot file을 추가로 포함할 수 있으며 같은 `schema_version=4`와 review metadata field를 사용합니다.
