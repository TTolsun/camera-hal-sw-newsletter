...
- `artifact-manifest.json`은 파일 이름 기준으로 manifest `files[]`에서 제외합니다 — 자기 자신뿐 아니라 **모든** artifact-manifest가 빠집니다. 자기 자신은 기록할 수 없고, 다른 날짜의 매니페스트는 기록할 수는 있지만 hash churn을 만듭니다(snapshot `files[]`는 여전히 해싱하고, date-scoped 매니페스트는 리뷰 패키지 writer가 두 번 씁니다). `00-review-guide.md`와 `release-qa-report.md`는 존재하면 `files[]`에 포함될 수 있지만 derived artifact라서 `missingRequired` 판단에는 사용하지 않습니다.
- Manifest surface는 두 가지입니다.
  ...
  - snapshot root `artifact-manifest.json`: workflow snapshot manifest입니다. `.tmp/**`, cache, debug file 같은 snapshot file을 추가로 포함할 수 있으며 같은 `schema_version=5`와 review metadata field를 사용합니다. 이 manifest의 `files[]`만 `size`·`sha256`을 유지합니다 — 그 배열의 heavy 항목(`debug_heavy`·`transient_attempt`) 값을 `retained_heavy_artifacts[]`가 그대로 받아 쓰기 때문에 배열 전체를 남깁니다. 그 heavy 부분집합은 커밋되지 않아 Git tree라는 정본이 없습니다. 반대로 같은 배열의 비-heavy 항목은 커밋되는 파일이라 Git tree 정본이 있고 소비자도 없으므로, date-scoped 매니페스트에서 없앤 것과 같은 종류의 사본이 아직 남아 있습니다(후속 정리 대상).
