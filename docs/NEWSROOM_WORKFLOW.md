...
- Manifest surface는 두 가지입니다.
  ...
  - snapshot root `artifact-manifest.json`: workflow snapshot manifest입니다. `.tmp/**`, cache, debug file 같은 snapshot file을 추가로 포함할 수 있으며 같은 `schema_version=5`와 review metadata field를 사용합니다. 이 manifest의 `files[]`도 `path`만 담습니다 — `size`·`sha256`을 읽는 소비자는 heavy 부분집합만 쓰는 `retained_heavy_artifacts[]` 하나뿐이었고, 나머지 비-heavy 항목은 커밋되는 파일이라 바이트 정본이 Git tree에 있기 때문입니다(#1018). 그래서 해싱 대상도 heavy 등급(`debug_heavy`·`transient_attempt`) 파일로 좁혔습니다. 그 heavy 부분집합만 커밋되지 않아 Git tree라는 정본이 없으므로 `retained_heavy_artifacts[]`가 계속 `size`·`sha256`을 기록합니다. 이로써 `schema_version=5`는 두 manifest surface에서 같은 `files[]` 모양 하나만 가리킵니다.
