...
- GitHub Actions에서 `03-newsroom-final-pr.yml`을 선택하고 `Run workflow`를 실행합니다.
...
- `validate-site.yml`은 quality/fact-check 문제를 non-blocking annotation으로 보고하고, structural validation만 blocking으로 처리합니다.
