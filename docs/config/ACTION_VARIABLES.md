...
| `NEWSROOM_EDITORIALPLAN_MODEL` | 선택 | `gemini-2.5-flash` | editorial assessment & planning stage(#700)의 primary model override입니다. | 내부 editorial plan의 분류·framing 품질을 특정 모델로 비교 검증할 때만 변경합니다. | editorial-plan stage는 best-effort이며 #700에서 기본 ON입니다(`NEWSROOM_EDITORIAL_PLAN_STAGE`, background-context와 동일하게 코드 기본값 `gemini`; `off`로 끌 수 있음). plan은 작성 framing 참고용일 뿐 deterministic source binding, source gap, forbidden bucket gate를 우회할 수 없습니다. Gemini Pro 계열 모델명은 validation error입니다. |
