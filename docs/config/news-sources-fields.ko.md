...
collector는 런타임 normalized source 객체와 `collected-news/YYYY-MM-DD/candidates.json`에는 계속 `section`과 `source_section`을 기록합니다. 다만 이 값은 입력 JSON의 중복 필드가 아니라 resolver가 `sectionMap`에서 계산한 값입니다.
