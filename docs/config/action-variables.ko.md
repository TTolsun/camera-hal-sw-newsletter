...
| `GEMINI_THINKING_BUDGET_JUDGE` | 선택 | `0` | public article judge stage의 Gemini thinking budget입니다. | semantic judge가 복잡한 article/checkpoint 경계를 놓친 artifact가 반복될 때만 조정합니다. | 기본 judge는 Flash Lite 비용 모델이므로 thinking budget을 높이는 것보다 prompt/input 축소를 먼저 검토합니다. |
