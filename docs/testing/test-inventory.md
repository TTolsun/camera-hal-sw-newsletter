...
| `tests/contract/editor-output-contract.test.js` | `contract` | Editor output schema, repair contract, diagnostic artifact. | Local temp/read helper. | `temp-artifact` | `tests/contract/` | Temp/read helper 공통화 후보입니다. |
...
| `tests/contract/fixture-policy.test.js` | `contract` | `good` fixture curated 여부, `bad` fixture PASS 금지, blocker flag PASS golden 금지. | `tests/helpers/fixture-loader`. | `none` | `tests/contract/` | Fixture ledger slice에서 유지/확장합니다. |
...
| `tests/contract/newsletter-quality.test.js` | `contract` | Quality gate hard blocker, article composition policy, source binding, linked evidence overclaim check. | `tests/helpers/quality-builders`, `quality` bad/good fixtures. | `minimized-regression` | `tests/contract/` | Large file입니다. helper와 fixture ledger 이후에만 분할합니다. |
| `tests/contract/newsroom-selection.test.js` | `contract` | Deterministic selection, eligibility, fallback composition, exclusion reason. | `fixture-loader`, `selection` bad/good fixtures, inline builders. | `minimized-regression` | `tests/contract/` | Large file입니다. builder extraction 후 분할합니다. |
| `tests/hygiene/policy-docs.test.js` | `hygiene` | Policy docs generated block marker와 drift detection. | Temp root. | `none` | `tests/hygiene/` | Temp helper 공통화 후보입니다. |
| `tests/hygiene/pr-template.test.js` | `hygiene` | Split PR template contract와 localization scan coverage. | Direct template read. | `none` | `tests/hygiene/` | Low-risk move 후보입니다. |
| `tests/contract/rendered-issue-structure.test.js` | `contract` | Rendered Markdown/HTML structural validator와 site validator agreement. | `tests/helpers/fs.js`, rendered fixture builder. | `temp-artifact` | `tests/contract/` | Local temp/write helper 중복은 Slice 7에서 제거했습니다. |
...
| `tests/workflow/selection-report-artifact.test.js` | `workflow` | Deterministic pre-LLM failure artifact와 selection report output. | Temp root, generated status artifact. | `temp-artifact` | `tests/workflow/` | Minimal temp artifact만 사용합니다. |
| `tests/workflow/source-effectiveness-report.test.js` | `workflow` | Source effectiveness report aggregation과 artifact writing. | `source-effectiveness` fixture, temp root. | `temp-artifact` | `tests/workflow/` | Fixture ledger에서 `workflow-shape`로 표시해야 합니다. |
...
| `tests/contract/stale-claims.test.js` | `contract` | Stale claim scrub와 retention behavior. | Inline source/section. | `none` | `tests/contract/` | Quality gate stale-claim blocker contract와 함께 둡니다. |
| `tests/workflow/targeted-retry.test.js` | `workflow` | Targeted retry, repair validation, source lock preservation, failed repair reviewability. | `tests/helpers/newsroom-builders`, inline sample. | `temp-artifact` | `tests/workflow/` | `workflow-scripts` helper 검토 후 helper extraction 후보입니다. |
...
| `tests/contract/validation-targets.test.js` | `contract` | Changed artifact date detection과 strict target-date selection. | Inline path string. | `path-contract` | `tests/contract/` | Path string 자체가 검증 대상입니다. |
| `tests/contract/validator-strictness.test.js` | `contract` | Historical vs strict `validate-site`와 `validate-quality` behavior. | `tests/helpers/fs.js`, minimal artifact. | `temp-artifact` | `tests/contract/` | Local temp/write helper 중복은 Slice 7에서 제거했습니다. |
| `tests/workflow/workflow-scripts.test.js` | `workflow` | PR body, publish status, fallback builder, reviewable artifacts, workflow wiring, publication annotation. | 많은 local temp/write/build helper. | `temp-artifact` | `tests/workflow/` | 가장 큰 파일입니다. helper extraction과 contract mapping 이후에만 분할합니다. |
