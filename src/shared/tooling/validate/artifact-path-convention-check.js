...
//
// schema_version은 이 차이를 표시하지 않는다(5가 두 규약에 모두 걸쳐 있다). 그래서 규약을 산문 대신
...
// 규약 판별 대상은 #262에서 articles/ 아래로 옮겨진 공개 출력물 중 이 검사가 실제로 보는 경로다.
// 이 검사가 보는 것은 content/·data/·newsletters/·sitemap.xml과 정적 사이트 surface인 assets/·css/다
// (AGENTS.md "디렉터리 구조" 참조). 같은 이동에 포함된 archive.html·robots.txt는 매니페스트에 기록된
// 적이 없어 아직 목록에 넣지 않았다. .tmp/·cache/·state/는 공개 출력물이 아니라 이동 대상이
// 아니었으므로 판별에 쓸 수 없다. 특히 state/는 루트 기준 매니페스트에만 나와 비교할 옛 형태가 아예 없다(옛 규약 시절
