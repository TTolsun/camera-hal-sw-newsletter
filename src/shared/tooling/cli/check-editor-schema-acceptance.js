...
// 스키마 거부 판정은 런타임이 발행을 막을 때 쓰는 함수 하나에만 맡깁니다. 여기서 조건을
// 더 두면 도구와 런타임의 답이 갈립니다. 일시 장애·타임아웃·잘못된 키는 이 함수가 false를
// 주므로 자동으로 "확인 못 함"이 됩니다 — 스키마 탓으로 단정하지 않는 쪽이 기본값입니다.
// (자격증명 패턴을 따로 먼저 거르는 분기를 뒀다가 뺐습니다. 결과가 같으면서, 오류 본문에
// 401·403 같은 숫자가 우연히 섞이면 진짜 스키마 거부를 가릴 수 있었습니다.)
function classifyProbeError(error) {
  return {
    schemaRejected: isSchemaComplexityError(error),
    message: redactReason(errorText(error))
  };
}
