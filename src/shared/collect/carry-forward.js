...
//   invalid         — artifact는 있으나 not_yet_eligible 스키마가 깨졌거나 파싱이 안 된다.
//   loaded          — 정상 로드(빈 목록도 loaded다 — "직전 주에 넘길 후보가 없었다"는 정상 결과다).
//
...
// 최신(YYYY-MM-DD 문자열이라 사전순 비교로 충분)을 고른다. 파싱 실패한 날짜 폴더는 다른 날짜의
// 정상 artifact를 가릴 이유가 없으므로 조용히 건너뛴다(그 폴더 자체가 lineage 매치였다면 대신
// invalid로 보고해야 하지만, coverage_week_key를 읽을 수조차 없으니 애초에 매치 여부를 판단할
// 수 없다 — 아래 findLineageCandidates 주석 참고).
...
function loadCarryForward({ root, coverage, carrySourcePathOverride = '', fs = nodeFs } = {}) {
  ...
  const lineage = findLineageCandidates(fs, root, previousWeekKey);
  if (!lineage) {
  ...
  if (!isValidCarryPayload(lineage.payload)) {
    return {
      status: 'invalid',
      candidates: [],
      carrySource: carrySourceFrom(root, lineage.filePath, lineage.raw, lineage.payload.run_mode)
    };
  }
