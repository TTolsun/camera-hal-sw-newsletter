...
module.exports = {
  ...
  //
  // 노출 없이도 해로운 방향(catalog에 있는데 표에 없음)은 잡을 수 있다. 미등록 stage가
  // 기본값 'editor'로 떨어지는 것이 부모 stage id -> scope 대응을 깨기 때문인데, 깨는
  // 방식이 둘이라 두 성질을 같이 봐야 한다(실측).
  //   - 기존 family에 새 접미사 파생이 미등록으로 생기면: 그 부모가 scope 두 개를 갖게 돼
  //     단일값이 깨진다. 단사는 그대로라 단사만 보면 놓친다.
  //   - 새 family가 통째로 미등록이면: 그 부모도 'editor'가 돼 base editor와 scope를
  //     공유하므로 단사가 깨진다.
  // 노출이 추가로 잡는 것은 반대 방향, 즉 표에만 남은 죽은 키다. 죽은 키는 런타임에
  // 무해하지만(조회는 stage id로만 들어온다) 다음 사람이 catalog에 있다고 오해할 근거가 된다.
