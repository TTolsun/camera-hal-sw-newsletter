...
function buildShortlistReport(date, collectedCandidates, options = {}) {
  ...
  return normalizeShortlistReport({
    ...
    // shortlist 전체를 후보 단위로 남긴다. 합계만으로는 '자격은 통과했는데 점수·슬롯에서
    // 밀린 후보'가 어느 것인지 커밋 이력만 보고 짚을 수 없다.
    //
    ...
    eligible_candidate_urls: shortlist
      .map(candidate => candidate.url || candidate.normalized_url || '')
      .filter(url => url !== ''),
