...
test('validate:images ignores a review-or-draft image audit report even on the strict target date', () => {
  // 커밋된 2026-05-28 리포트가 정확히 이 모양이다(count=1, mode=review-or-draft). 발행 대상이
  // 아니었던 주의 잔존 카운트로 발행을 막으면 안 된다.
  withFixture(
    ...
    result => {
      assert.equal(
        result.status,
        0,
        `a review-or-draft audit report must not block publication. stderr=${result.stderr}`
      );
    }
  );
});
