...
// 돌려준다(2026-09-26 run 36238083997: 기사 10편 전부 실제 개행 0개, 이 표기 8개). 그 본문은
// 문단 하나로 세어져 발행이 막히므로 이 표기를 줄바꿈으로 읽는다.
//
// 단, 실제 개행이 하나도 없을 때만 그렇게 읽는다. 관측된 실패는 전부 그 모양이었고, 실제 개행이
// 있는 정상 본문에서는 `C:\new`처럼 역슬래시 n으로 시작하는 글자열이 산문일 수 있어 무조건
// 바꾸면 글자가 사라진다.
const ESCAPED_LINE_BREAK_PATTERN = /\\r\\n|\\n/g;

function decodeEscapedLineBreaks(value) {
  if (/\r|\n/.test(value)) return value;
  return value.replace(ESCAPED_LINE_BREAK_PATTERN, '\n');
}

function normalizeBodyMarkdown(value) {
  if (typeof value !== 'string') return '';
  const lines = decodeEscapedLineBreaks(value)
    .replace(/\r\n?/g, '\n')
    .split('\n')
...
// 문단 사이의 빈 줄은 JSON 문자열 안의 실제 줄바꿈 문자여야 한다. 개행이 전혀 없는 본문은
// normalizeBodyMarkdown이 역슬래시 n 표기를 줄바꿈으로 되돌리지만, 프롬프트도 같은 요구를 말해야 한다.
const BODY_MARKDOWN_LINE_BREAK_RULE =
  '문단 사이의 빈 줄은 JSON 문자열 안의 실제 줄바꿈 문자로 넣으세요. 역슬래시와 n을 글자 두 개로 이어 쓴 표기로 줄바꿈을 대신하지 마세요.';
