...
test('completion validation accepts a single missing article section', () => {
  const sections = validateCompletionSections({
    sections: [{
      ...
      article_sections: {
        verified_facts: ['CameraX 1.5 release is the source event.'],
        background_context: 'CameraX sits above camera2 and exposes compatibility regressions that can map back to HAL behavior.',
        hal_driver_impact: 'Check request/result metadata, stream combinations, and Camera ITS deltas on representative devices.',
        action_items: ['Within 2 weeks, assign a camera owner to compare Camera ITS logs before and after CameraX 1.5.'],
        team_share_points: 'Use CameraX 1.5 as a concrete compatibility validation trigger.'
      },
      is_ai_related: false,
