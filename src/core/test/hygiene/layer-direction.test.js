...
test('LAYER_RANK encodes one-way direction core<collector<discovery<generator', () => {
  assert.equal(LAYER_RANK.core, 0);
...
test('layerOf maps src/<layer>/... paths and ignores non-layer paths', () => {
  assert.equal(layerOf('src/core/domain/x.js'), 'core');
...
test('a generator -> core import passes (downstream depending on upstream)', () => {
  assert.equal(violationsFor('src/generator/reporter/x.js', 'core'), false);
});
...
test('a same-layer import passes', () => {
  assert.equal(violationsFor('src/core/domain/a.js', 'core'), false);
});

test('a core -> generator import is a violation (upstream depending on downstream)', () => {
  assert.equal(violationsFor('src/core/domain/a.js', 'generator'), true);
});
...
test('isProductionLayerFile excludes test and tooling code', () => {
  assert.equal(isProductionLayerFile('src/core/domain/a.js'), true);
  assert.equal(isProductionLayerFile('src/generator/reporter/b.js'), true);
  assert.equal(isProductionLayerFile('src/core/test/hygiene/c.test.js'), false);
  assert.equal(isProductionLayerFile('src/generator/test/contract/d.test.js'), false);
  assert.equal(isProductionLayerFile('src/core/tooling/check-layer-direction.js'), false);
...
test('evaluate: a baselined violation passes (no new edge)', () => {
  const current = ['src/core/a.js -> src/generator/b.js'];
  const baseline = ['src/core/a.js -> src/generator/b.js'];
...
test('evaluate: a NEW violation not in baseline fails (added is non-empty)', () => {
  const current = [
    'src/core/a.js -> src/generator/b.js',
    'src/core/new.js -> src/generator/c.js'
  ];
  const baseline = ['src/core/a.js -> src/generator/b.js'];
  const { added } = evaluate(current, baseline);
  assert.deepEqual(added, ['src/core/new.js -> src/generator/c.js']);
});

test('evaluate: a baseline entry no longer present is reported as stale, not failed', () => {
  const current = [];
  const baseline = ['src/core/resolved.js -> src/generator/x.js'];
  const { added, stale } = evaluate(current, baseline);
  assert.deepEqual(added, []);
  assert.deepEqual(stale, ['src/core/resolved.js -> src/generator/x.js']);
});
