...
설정, 사이트, 외부 이미지, 품질, 한글화 검증을 실행합니다. 발행 전 필수 gate입니다.

```powershell
npx serve .
```

정적 사이트를 로컬 HTTP 서버로 띄워 브라우저 `fetch()`가 JSON을 읽을 수 있게 합니다.
...
별도 unit test suite가 많지 않으므로 `npm.cmd run validate`를 권위 있는 검증 명령으로 취급합니다. 발행 산출물에는 `TODO`가 없어야 하며, 필수 섹션과 참조, 기사 이미지 fallback 규칙, deterministic quality threshold를 만족해야 합니다.
...
발행 위험을 숨기기 위해 검증을 약화하지 않습니다. 생성 결과에 편집 수정이 필요하면 review artifact를 보존합니다. 외부 이미지를 임의 URL로 대체하지 말고 resolver와 로컬 fallback 계약을 사용합니다.
