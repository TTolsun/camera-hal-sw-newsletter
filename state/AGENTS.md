...
- catch-up 창을 넓혀 **실행일 기준 창**이 `coverage_starts_at` 이전으로 넘어가면, 그 구간에는 게이트가 볼 레코드가 없어 catch-up이 과거 main 기사를 다시 올립니다. 필터가 `days_since_published`를 벽시계로 재기 때문에 창은 시간이 지날수록 저절로 뒤로 물러납니다. 또 `catchUpPolicy.maxAgeDays`는 `selectionWindowPolicy.referenceContextDays`를 넘을 수 없으므로(`src/shared/common/newsletter-policy.js`) 단독 상향은 validator가 막습니다. 두 값을 함께 올리는 변경만 이 계약을 건드릴 수 있고, 그때 함께 봐야 합니다.
