...
- 커밋된 과거 호의 `article_exposure_coverage`는 그 호를 발행하던 시점의 사본이라, 이 정정 이전에 발행된 호에는 옛 선언이 그대로 남아 있습니다. 정본은 이 state 파일 하나뿐이고 과거 산출물은 재작성하지 않습니다(아카이브 전방-전용). 과거 호를 리플레이할 때 그 호의 `generation-status.json`이나 `selection-report.json`으로 커버리지를 판단하지 마세요 — #1037이 고친 오판을 한 층 아래에서 되풀이하게 됩니다.
