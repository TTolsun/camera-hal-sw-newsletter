...
test('schedule cutover leaves only the daily auto PR workflow on the newsroom schedule', () => {
  ...
  const dailyAuto = fs.readFileSync(path.join(workflowDir, 'newsroom-daily-auto-pr.yml'), 'utf8');
  ...
  assert.deepEqual(scheduledWorkflowFiles, ['newsroom-daily-auto-pr.yml']);
