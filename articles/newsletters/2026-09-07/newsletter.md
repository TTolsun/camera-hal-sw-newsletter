...
이번 주 뉴스레터에서는 Linux 미디어 하위 시스템 및 libcamera 프로젝트에서 진행 중인 핵심 카메라 드라이버 및 이미지 파이프라인 패치 소식을 다룹니다. OmniVision OG0VA1B 흑백 센서 드라이버 v6, Qualcomm x1e/Hamoa 플랫폼용 카메라 DTS v6, Lenovo Yoga Book 카메라 지원 v7 패치와 함께 libcamera 컨트롤 저장소의 복사 단위화 준비 패치와 소프트웨어 ISP 정지 경로 오류 수정 패치 등 하위 스택 엔지니어들이 주목해야 할 실무 변경 사항들을 상세히 분석합니다.



## 1. 이번 주 3줄 브리핑

- OmniVision OG0VA1B 흑백 CMOS VGA 센서 드라이버 패치(v6)가 공개되어, 단일 레인 MIPI CSI-2를 통한 10비트 RAW (Y10) 프레임 출력 지원이 제안되었습니다.
- Qualcomm x1e/Hamoa 플랫폼용 카메라 DTSI 패치(v6)에서 CAMSS 노드의 데이터 레인 정렬 및 전원 공급 스키마 정합성 개선이 확인되었습니다.
- libcamera 및 AtomISP 기반 Lenovo Yoga Book 카메라 지원 패치(v7)와 함께, libcamera 컨트롤 스토리지 구조를 정리하는 패치와 소프트웨어 ISP의 정지 경로 오류를 고치는 패치가 제출되었습니다.
...
v6에서 바뀐 것은 CAMSS 노드 내 데이터 레인 시작 인덱스를 기존 0에서 1로 조정하여 물리 계층(PHY)과 정렬을 맞춘 한 가지입니다. 전원 공급 노드 정의를 vdda-0p8-supply에서 vdda-0p9-supply로 옮겨 스키마 요구사항에 맞춘 변경은 앞선 v5에서 이루어졌습니다.
...
libcamera 내부의 컨트롤 스토리지 유니온 구조체에 명확한 이름을 부여하는 v3 패치가 제출되어 accepted 상태로 전환되었습니다. 커밋 메시지가 밝힌 목적은 활성 멤버가 무엇이든 저장소를 한 단위로 복사할 수 있게 만드는 것이며, 이 패치는 이동 생성자와 swap() 연산을 도입하는 5패치 시리즈의 첫 번째입니다.
...
유니온 멤버에 이름을 붙인 이유는 활성 멤버와 무관하게 저장소를 한 덩어리로 복사하기 위해서입니다. 같은 패치가 멤버 이름을 internal과 external로 바꾸어 제자리 저장과 동적 할당 저장을 구분하고, 불필요해진 reinterpret_cast 일부도 함께 제거했습니다. 이 패치는 Move constructor/assignment + swap 시리즈(v3, 5패치)의 첫 조각이고, 나머지 네 패치는 비트 필드 제거, 이동 생성자와 대입 연산자 구현, swap() 구현, ControlList::set()의 rvalue 오버로드 추가입니다.
...
이 조각 자체는 저장소 표현을 정리하는 단계라 HAL 런타임 동작을 바꾸지 않습니다. 다만 libcamera 소스를 커스텀하여 Android HAL 하위 계층에 연동하는 프로젝트라면, 유니온 명명 변경으로 기존 커스텀 제어 코드에서 컴파일 에러가 나지 않는지 빌드 호환성을 점검해야 합니다.
...
libcamera의 소프트웨어 이미지 신호 처리(software_isp) 컴포넌트에서 카메라 시작이 실패했을 때 호출이 무한정 멈추던 문제를 고치는 패치가 제출되었습니다. 아직 구동되지 않은 워커 스레드를 기다리지 않고 일찍 반환하도록 바꾸어, 원래의 캡처 오류가 호출자에게 그대로 전달되게 합니다.

2026년 9월 6일, Birk Skyum은 libcamera 패치 트래커를 통해 software_isp의 정지 경로를 고치는 패치를 제출했습니다. software_isp는 하드웨어 ISP가 없는 시스템에서 CPU 리소스를 활용해 이미지 처리를 수행하는 핵심 모듈입니다.

패치 설명에 따르면, simple 파이프라인은 캡처 장치가 스트리밍 시작에 실패할 경우 SoftwareIsp::start()가 실행되기 전에 stop()을 호출합니다. 이때 블로킹 방식으로 불린 Debayer::stop()이 아직 구동되지 않은 워커 스레드를 무한정 기다리면서 호출 자체가 반환되지 않습니다. 이번 패치는 워커가 시작되지 않은 상태라면 조기에 반환하도록 바꾸어 이 대기를 없앱니다.

그 덕분에 Camera::start()가 원래의 캡처 오류를 호출자에게 보고할 수 있게 됩니다. 패치에는 freedesktop GitLab의 libcamera 이슈 349번이 근거로 연결되어 있으며, 패치 트래커에서의 현재 상태는 superseded로 후속 리비전이 올라와 있습니다.

### Camera HAL/Driver 관점에서의 의미

직접적인 HAL 변경은 없으나, 소프트웨어 ISP를 경유하는 파이프라인에서 카메라 열기가 실패했는데도 호출이 반환되지 않고 멈추는 증상을 겪었다면 이 경로를 의심해 볼 수 있습니다. 캡처 시작 실패를 일부러 주입하는 시나리오로 오류가 상위 계층까지 전달되는지 확인하시기 바랍니다.
