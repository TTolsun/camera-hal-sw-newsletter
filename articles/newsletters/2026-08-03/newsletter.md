# AOSP Camera / Driver / SoC Platform 기술 뉴스레터 (2026-08-03)
...
- Linux 커널 V4L2 서브시스템에 Himax HM1092, onsemi AR0234, OmniVision OG0VA1B 등 신규 이미지 센서 드라이버 패치가 제출되어 하드웨어 지원이 확장되고 있습니다.
- Qualcomm CAMSS(카메라 서브시스템)에 MIPI C-PHY 구성 지원을 추가하는 패치(v9)가 제안되어 고대역폭 스트림의 전력 및 성능 최적화 기반이 마련되었습니다.
- 신규 센서 드라이버와 물리 계층 변경은 Camera HAL에 직접적인 API 변경을 요구하지 않으나, 이미지 파이프라인 초기화 및 프레임 타이밍 검증이 필요합니다.
