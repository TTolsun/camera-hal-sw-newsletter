...
| 분류 체계 누락 | `taxonomy_missing` | true | 7 camera-relevant candidate(s) were not mapped to a known camera bucket. |
| Fallback 기사만 남음 | `fallback_only_composition` | false | 진단 신호 없음 |
| Source discovery 중복 또는 무효 | `duplicate_or_noop_source_discovery` | true | Gemini discovery produced 5 candidate(s) but gemini_new_unique_url_count=0. |
...
| lore.kernel.org linux-media list | 8 | 3 | finalSelectionEligibility=exclude; main_eligible=false | 소스 유지, 파서 수정 |
| libcamera Patchwork (patch review) | 5 | 1 | main_eligible=false; source_gap_risk=true | 소스 유지, 파서 수정 |
...
| Unregistered source: android-version-features | 1 | 0 | Review source-change-events artifacts before using this candidate.; Source snapshot event is review/watchlist only because date evidence is weak or diagnostic. | 소스 유지, 파서 수정 |
...
| 소스 유지, 파서 수정 | `KEEP_AND_FIX_PARSER` | lore.kernel.org linux-media list | Official or high-priority source has parser/source-evidence issues but should stay enabled. | high |
...
| --- | --- | --- | --- |
| missing_optional_artifact | articles/content/newsroom/2026-08-17/evidence-pack-summary.json not found; partial diagnosis will continue. | articles/content/newsroom/2026-08-17/evidence-pack-summary.json |  |
