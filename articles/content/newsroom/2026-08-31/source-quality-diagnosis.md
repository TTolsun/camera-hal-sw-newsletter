...
| 분류 체계 누락 | `taxonomy_missing` | true | 11 camera-relevant candidate(s) were not mapped to a known camera bucket. |
| Fallback 기사만 남음 | `fallback_only_composition` | false | 진단 신호 없음 |
| Source discovery 중복 또는 무효 | `duplicate_or_noop_source_discovery` | true | Gemini discovery produced 11 candidate(s) but gemini_new_unique_url_count=0. |
...
| lore.kernel.org linux-media list | 8 | 5 | main_eligible=false; source_gap_risk=true | 소스 유지, 파서 수정 |
| libcamera Patchwork (patch review) | 8 | 2 | main_eligible=false; source_gap_risk=true | 소스 유지, 파서 수정 |
...
| Anthropic News | 3 | 0 | Generic technology item without article-level camera, driver, SoC, or native tooling evidence; keep as watchlist/briefing material.; briefing_only=true | 소스 유지, 파서 수정 |
...
| Unregistered source: aosp-camera-its-release-notes | 1 | 0 | Review source-change-events artifacts before using this candidate.; Source snapshot event is review/watchlist only because date evidence is weak or diagnostic. | 소스 유지, 파서 수정 |
...
| 소스 유지, 파서 수정 | `KEEP_AND_FIX_PARSER` | Unregistered source: aosp-camera-its-release-notes | Official or high-priority source produced camera-relevant candidates, but parser/source-extraction-like rejection reasons blocked eligibility. | high |

## 경고

| 유형 | 메시지 | Source artifact | 심각도 |
| --- | --- | --- | --- |
| missing_optional_artifact | articles/content/newsroom/2026-08-31/evidence-pack-summary.json not found; partial diagnosis will continue. | articles/content/newsroom/2026-08-31/evidence-pack-summary.json |  |
