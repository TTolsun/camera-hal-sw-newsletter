...
- `size`·`sha256`은 담지 않습니다(`schema_version` 4부터). 커밋되는 파일의 실제 바이트는 **Git tree가 정본**이므로 매니페스트가 두 번째 정본을 만들지 않습니다. 파일이 그 run에서 어떤 바이트였는지 알아야 하면 매니페스트가 아니라 해당 커밋의 Git tree를 보세요.
