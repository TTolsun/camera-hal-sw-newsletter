...
- **나브 라벨은 페이지마다 다른 곳에서 잠근다.** 홈은 `homepage-archive.test.js`, 아카이브는 `archive-page.test.js`, Lab 페이지는 `learning-page.test.js`가 디스크의 HTML을 읽어 `홈 / 아카이브 / GitHub`와 그 링크 대상을 잠근다(공용 헬퍼 `src/shared/test/helpers/site-nav.js`, `<header>` 안으로 스코프된다). **푸터 라벨은 어디에서도 잠기지 않는다** — 푸터의 홈·아카이브가 영어로 돌아가도 잡는 검사가 없다. 이슈 페이지는 `newsletter-renderer.test.js`가 잠그는데 **그것은 생성기 출력만 본다** — 이미 커밋된 이슈 HTML 51개의 나브를 읽는 게이트는 없다. 그래서 새로 렌더되는 호는 보호되고 과거 발행물은 그대로 남는다. `validate-site.js`에도 같은 일을 하는 `validateSiteNavLabels`가 있었지만 `<nav class="site-nav">` 나 `<header data-site-header>` 를 찾았고 리디자인 이후 실제 마크업은 `<header class="site-header">` + `<div class="nav-links">` 라 **53개 전부 매치하지 않았다** — 잠그는 것이 없으면서 게이트 목록에 이름만 올라 있어서 걷어냈다. 같은 함수 안에 있던 `data-site-header` 페이지의 `site-header.js` 로드 검사도 함께 빠졌다; `assets/js/site-header.js`는 홈·아카이브가 로드하지만 `data-site-header` 를 쓰는 페이지가 0개라 현재 아무것도 mount 하지 않는다. 그 컴포넌트를 실제로 채택하면 로드 검사를 다시 세워야 한다.
- **발행된 이슈 페이지 27개는 나브가 영어다**(`Home / Archive / GitHub`). 그중 26개가 `newsletters.json`에 실려 있고 나머지 하나가 고아 `2026-06-02`다. **라벨만 다른 것이 아니다** — 이 페이지들의 헤더는 브랜드까지 구버전이라 워드마크가 "Camera SW Newsletter"이고 `HALley-logo.png`가 없다(현행은 "Camera SW Newsroom" + 로고). 프리즈된 페이지라 스스로 바뀌지 않으므로 게이트 문제는 아니지만, 공개 사이트에 두 세대의 헤더가 공존하는 것은 사실이다. 표적 패치(#940 전례)로 고친다면 라벨 두 개가 아니라 헤더 블록 전체를 맞춰야 한다.
