...
- **`validate-site.js`는 공개 표면 53개를 본다** — 홈·아카이브, dated 33개, 위클리 17개, 그리고 AI Engineering Lab. 한동안 `htmlFiles`가 `articles/data/newsletters.json` 항목으로만 만들어져 dated 33개(총 35개)만 돌았고, 그래서 anchor 균형·TODO 검출이 **현재 발행 레인 전체(W19~W35)** 와 Lab 페이지에 적용되지 않았다. 지금은 `newsletters-weekly.json`도 함께 읽고, 그 파일이 없거나 목록의 페이지가 디스크에 없으면 fail 한다 — 조용히 건너뛰면 그 순간부터 위클리가 다시 무검사로 나가기 때문이다. Lab 페이지는 `newsletters/` 접두어가 아니라 이슈 전용 검사(`issue-briefing`·`source-list` 등)는 받지 않고 anchor·TODO만 받는다. 나머지는 `learning-page.test.js`(CSS 불변식과 키보드 도달성), `homepage-archive.test.js`(푸터 링크), `validate-seo-metadata.js`(메타·sitemap)가 덮는다.
- **나브 라벨 계약(`validateSiteNavLabels`)은 현재 어느 페이지에도 걸리지 않는다.** `<nav class="site-nav">` 또는 `<header data-site-header>` 를 찾는데, 리디자인 이후 실제 마크업은 `<header class="site-header homepage-site-header">` + `<div class="nav-links">` 라 53개 전부 조기 return 한다. 검사를 `nav-links` 까지 넓히면 레거시 daily 26개가 영어 라벨(Home / Archive / GitHub)로 실패하므로, 그 26개를 어떻게 할지 정하는 것이 먼저다(재렌더 소스가 없어 수용된 예외다).
