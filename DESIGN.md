...
- **`validate-site.js`는 공개 표면 53개를 본다** — 홈·아카이브, dated 33개, 위클리 17개, 그리고 AI Engineering Lab. 한동안 `htmlFiles`가 `articles/data/newsletters.json` 항목으로만 만들어져 dated 33개(총 35개)만 돌았고, 그래서 anchor 균형·TODO 검출·나브 라벨 계약이 **현재 발행 레인 전체(W19~W35)** 와 Lab 페이지에 적용되지 않았다. 지금은 `newsletters-weekly.json`도 함께 읽고, 그 파일이 없으면 fail 한다 — 조용히 빈 목록이 되면 그 순간부터 위클리가 다시 무검사로 나가기 때문이다. Lab 페이지는 `newsletters/` 접두어가 아니라 이슈 전용 검사(`issue-briefing`·`source-list` 등)는 받지 않고 anchor·TODO·나브만 받는다. 나머지는 여전히 `learning-page.test.js`(CSS 불변식과 키보드 도달성), `homepage-archive.test.js`(푸터 링크), `validate-seo-metadata.js`(메타·sitemap)가 덮는다.
