...
- **미사용(dead) CSS 정리 (부분 완료)**: 독립 규칙 `.headline-*`·`.latest-newsletter-card`·`.latest-card-*`·`.newsletter-card`·`.card-summary`·`.clamp-3`은 제거함(styles.css 약 231줄 감소, 홈·아카이브 스크린샷 byte-identical=시각 회귀 없음). 남은 `.homepage .site-hero`·`.hero-*`·`.issue-hero-mascot`는 살아있는 archive/issue와 공유되는 grouped·test-lock 셀렉터(예: `.site-hero::before, .archive-hero::before, .issue-hero::before` 글로우, `.hero-mascot img, .archive-hero-mascot img, ...` / `.archive-hero-mascot, .issue-hero-mascot` 그룹)에 얽혀 있어, 셀렉터 분리 + issue CSS-lock 테스트(`homepage-archive.test.js` issue 블록·:524 shell 블록) 갱신을 동반하는 신중한 후속 증분으로 남긴다. (참고: `.newsletter-card`는 `exactSelectorBlock('.archive-card')`가 image-forward 카드 블록으로 해석되도록 text-decoration 그룹 한 곳에만 의도적으로 dead-selector로 남김.)
