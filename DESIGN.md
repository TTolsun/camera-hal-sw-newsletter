...
- **잠금은 `src/shared/test/workflow/learning-page.test.js`에 있다.** 두 갈래다 — 레이아웃 불변식 둘과, #1009가 더한 키보드 도달성 계약 일곱(모든 가로 스크롤 컨테이너를 "도달 가능" 또는 "면제"로 분류하고, 도달 가능한 것에는 이름 붙은 tab stop을, scoreboard에는 landmark를 요구하며, 코드·트리 블록은 landmark로 만들지 않고, 결과 표는 표로 남기고, 초점 대상이 2단 sticky 헤더에 가리지 않게 한다). 레이아웃 쪽 둘은 이렇다. (1) `learning.css`의 모든 유연 grid 트랙은 `minmax(0, …)`로 쓴다 — `1fr`은 `minmax(auto, 1fr)`이라 트랙이 내용의 최소 폭 아래로 못 줄어드는데, 이 카드들 안에는 그 최소 폭이 휴대폰 화면보다 넓은 것(`min-width: 720px`인 `.scoreboard` 표, `white-space: pre`인 `.artifact-tree`)이 있어 페이지 전체가 가로로 스크롤된다. 0이 아닌 하한도 같이 막는다. (2) `@media (max-width: 780px)`에서 카드 그리드 4종(`.learning-grid`·`.source-grid`·`.week-columns`·`.week-card`)이 `minmax(0, 1fr)` 한 열로 접힌다. `.loop-flow`는 의도적으로 빠져 있다 — 단계 흐름이라 그 폭에서 3열을 유지한다.
...
- **`validate-site.js`는 공개 표면 53개를 본다** — 홈·아카이브, dated 33개, 위클리 17개, 그리고 AI Engineering Lab. 한동안 `htmlFiles`가 `articles/data/newsletters.json` 항목으로만 만들어져 dated 33개(총 35개)만 돌았고, 그래서 anchor 균형·TODO 검출이 **현재 발행 레인 전체(W19~W35)** 와 Lab 페이지에 적용되지 않았다. 지금은 `newsletters-weekly.json`도 함께 읽고, 목록에 있는데 디스크에 없는 위클리 페이지는 fail 한다(dated와 같은 계약). **인덱스 자체의 존재·JSON·배열 검사는 validate-site가 하지 않는다** — `rendered-issue-structure.js`의 `NEWSLETTER_INDEX_PATHS`가 두 인덱스에 대해 하고, validate-site는 그 결과에 기댄다. 다만 그 검사는 dated 호가 하나라도 있을 때만 돌아서(`validateDataIndex: index === 0`) dated 인덱스가 비면 위클리 인덱스는 무검사가 된다(현재 dated 33개라 항상 돈다). Lab 페이지는 `newsletters/` 접두어가 아니라 이슈 전용 검사(`issue-briefing`·`source-list` 등)는 받지 않고 anchor·TODO만 받는다. 나머지는 `learning-page.test.js`(CSS 불변식과 키보드 도달성), `homepage-archive.test.js`(푸터 링크), `validate-seo-metadata.js`(메타·sitemap)가 덮는다.
- **나브 라벨은 페이지마다 다른 곳에서 잠근다.** 홈은 `homepage-archive.test.js`, 아카이브는 `archive-page.test.js`, 렌더된 이슈 페이지는 `newsletter-renderer.test.js`, Lab 페이지는 `learning-page.test.js`가 각각 `홈 / 아카이브 / GitHub`를 잠근다. `validate-site.js`에도 같은 일을 하는 `validateSiteNavLabels`가 있었지만 `<nav class="site-nav">` 나 `<header data-site-header>` 를 찾았고 리디자인 이후 실제 마크업은 `<header class="site-header">` + `<div class="nav-links">` 라 **53개 전부 매치하지 않았다** — 잠그는 것이 없으면서 게이트 목록에 이름만 올라 있어서 걷어냈다. `assets/js/site-header.js`(홈·아카이브가 로드한다)도 `data-site-header` 를 쓰는 페이지가 0개라 현재 아무것도 mount 하지 않는다; 그 컴포넌트를 실제로 채택하면 로드 검사를 다시 세워야 한다.
- **레거시 daily 26개는 나브가 영어다**(`Home / Archive / GitHub`). 마크업은 나머지 27개와 같고 라벨 두 개만 다르다. 프리즈된 페이지라 스스로 바뀌지 않으므로 게이트 문제는 아니지만, 공개 사이트에 두 언어의 나브가 공존하는 것은 사실이다 — 고치려면 표적 패치(#940 전례)로 라벨만 바꾼다.
