...
- **`validate-site.js`가 보는 이슈 페이지는 dated 33개뿐이다 — 위클리 17개와 Lab 페이지는 밖이다.** `htmlFiles`는 홈·아카이브와 `articles/data/newsletters.json` 항목으로만 만들어지는데(현재 2 + 33 = 35개), 그 33개가 전부 dated 항목이다. 위클리는 `newsletters-weekly.json`에 있고 어떤 검증기도 그 이슈 페이지 HTML을 열지 않는다 — 즉 anchor 균형·TODO 검출·셸/스크립트/링크 계약이 **현재 발행 레인 전체(W19~W35)** 에 적용되지 않는다. Lab 페이지도 같은 이유로 밖이다. 그래서 그 게이트가 잠그는 셸·스크립트 로드·링크 계약은 Lab 페이지에 적용되지 않는다. 이 페이지를 덮는 것은 `learning-page.test.js`(CSS 불변식과 키보드 도달성), `homepage-archive.test.js`(푸터 링크), `validate-seo-metadata.js`(메타·sitemap) 셋이다.
